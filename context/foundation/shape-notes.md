...
checkpoint:
  ...
  gray_areas_resolved:
    ...
    - topic: primary persona scope
      decision: single hobbyist, initially the user themselves
    - topic: product insight
      decision: one entry combines model origin, paint recipe, ordered steps, and photos
    - topic: access model
      decision: login required
    - topic: role model
      decision: flat model; authenticated user sees and edits only their own entries
    - topic: MVP timeline
      decision: full described MVP targets 3 weeks of after-hours work
    - topic: paint card display
      decision: step paint cards show paint name and color; application type is not required in MVP card display
    - topic: paint application type
      decision: application type such as base/layer/wash/drybrush is removed from MVP
    - topic: entry type
      decision: entry type removed from MVP because it is unclear
    - topic: entry status
      decision: new entries default to draft; status changes to ready or similar after publish/mark-ready action
    - topic: model origin
      decision: MVP stores model origin as a custom note only
    - topic: step title
      decision: step title removed from MVP; ordered description is enough
    - topic: step photo
      decision: step photo is optional and limited to one photo per step in MVP
    - topic: business logic
      decision: entry paint list is the source for step paint assignments; entries move through draft -> ready/published workflow
    - topic: non-functional requirements
      decision: private-by-default owner access; save operation gives clear confirmation or error
    - topic: product type
      decision: web app
    - topic: target scale
      decision: MVP is for the user and a dozen-plus people; mapped to medium scale
    - topic: timeline
      decision: no hard deadline; after-hours work
    - topic: scale insight
      decision: at 100x scale, search and filtering become the first pressure point
  frs_drafted: 12
...
## Forward: product-scale

If usage grows by roughly 100x, advanced search, filtering, and tags become the first likely pressure point. They remain outside MVP.

## Quality cross-check

All greenfield quality elements present on 2026-05-21: Access Control, one-sentence Business Logic, project artifact, three-week timeline budget, and Non-Goals.
